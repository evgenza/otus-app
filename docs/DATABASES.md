...
Счетчик в фиксированном минутном окне на Valkey: middleware выполняет
Lua-скрипт, который атомарно на стороне сервера инкрементирует ключ
`rl:<ip>:<окно>` и первым же запросом вешает на него TTL окна, и отдает 429
при превышении. Атомарность скрипта покрыта тестами на miniredis (он
исполняет тот же Lua): TTL ставится ровно один раз, повторные запросы окно
не продлевают, после истечения окна счет начинается заново. IP берется из X-Forwarded-For - в проде его выставляет nginx,
