...
Дашборд Grafana (провижнится автоматически): RPS по сервисам, p95 времени
...
пароль приложения). Проверка отправки:

```
$ python3 -c "import smtplib; ..."   # прямая проверка SMTP
SENT OK
```

Триггер алерта: останавливаем gateway → через 30с `ServiceDown` переходит в
`firing`, Alertmanager отправляет письмо; после запуска gateway приходит
письмо `RESOLVED`.

```
ServiceDown firing gateway   →   письмо на почту
```
