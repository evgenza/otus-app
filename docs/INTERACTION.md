...
$ curl -X POST localhost:8090/gw/messages -d '{"text":"повтор с идемпотентностью"}'
# сообщений было: 4, стало: 5 — прокси повторил запрос,
# но app вернул уже сохранённое сообщение по ключу идемпотентности
# лог app: "повторный запрос с тем же ключом идемпотентности"
```
